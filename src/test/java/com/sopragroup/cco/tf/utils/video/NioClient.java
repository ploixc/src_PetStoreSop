...
public class NioClient implements Runnable {
	...
	public static void testVlc(boolean fautQuitter) {
		try {
			
			String ficScreen = "D:\\usr\\vdaburon\\dev\\workspace3.7\\gestdoctf\\bin\\capture223.mp4";
			String ficOut = "D:\\usr\\vdaburon\\dev\\workspace3.7\\gestdoctf\\bin\\capture_" + System.currentTimeMillis() + ".mp4";
			
			NioClient client = new NioClient(InetAddress.getByName("localhost"), 4444);
			Thread t = new Thread(client);
			t.setDaemon(true);
			t.start();
			RspHandler handler = new RspHandler();
...
//			handler.waitForResponse();
			...
			cmd = "is_playing";
			client.send((cmd + "\n").getBytes(), handler);
			retour = handler.waitForResponseString();
